...
parser.add_argument("source_dir", type=str, help="Directory to resize")
parser.add_argument("destination_dir", type=str, help="Destination directory")
parser.add_argument(
    "--hevc", action="store_true", help="Decode HEVC file. If False, decode h264."
)
parser.add_argument(
    "--resize", action="store_true", help="Resize the video. Set --height."
)
parser.add_argument(
    "--height", type=int, default=720, help="Output resolution (height)."
)
parser.add_argument(
    "--cpu", action="store_true", help="Use CPU instead of GPU to encode videos."
)
parser.add_argument(
    "--crf", type=int, default=21, help="crf (quality) for CPU encoding."
)
parser.add_argument(
    "-v",
    "--video_bitrate",
...
parser.add_argument(
    "-a", "--audio_bitrate", type=int, default=128, help="Kilo-bitrate for audio"
)
parser.add_argument(
    "-s", "--audio_samplerate", type=int, default=48000, help="Sampling rate for audio"
)
parser.add_argument(
    "-r",
    "--resample",
...
parser.add_argument(
    "--ext", type=str, nargs="*", default=["mp4"], help="Video file extensions to find."
)
                    ...
                    if args.cpu:
                        ...
                        ffmpeg_cmd_video = [
                            *ffmpeg_cmd_nvdecode,
                            ...
                            "10",
                            "-b:v",
                            "%dk" % args.video_bitrate,
                            "-maxrate:v",
                            "%dk" % (args.video_bitrate * 2),
                            "-profile:v",
                            "high",
                            *ffmpeg_youtube_recommended,
                            *COLOUR_RANGE_FULL,
                        ...
                        ffmpeg_cmd_resize = [
                            "-vf",
                            f"scale_npp=-2:{args.height:d}:interp_algo=super",
                        ]

                    # -map 0 to copy all audio streams (and possibly metadata?)
                    # libfdk_aac codec has a higher quality (but defaults to a low-pass filter of 14kHz),
                    # so consider using it in case you have it enabled.
                    ffmpeg_cmd_audio_aac = [
                        "-c:a",
                        "aac",
                        "-b:a",
                        "%dk" % args.audio_bitrate,
                        "-ar",
                        "%d" % args.audio_samplerate,
                        "-profile:a",
                        "aac_low",
                    ]
