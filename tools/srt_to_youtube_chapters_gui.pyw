...
class SrtToYTChapters:
    def __init__(self, root_window):
        ...
        self.btn_open_srt = tk.Button(
            self.root, text="Open SRT", command=self._on_click_open_srt
        )
        self.label_offset_hour = tk.Label(self.root, text="offset (hour)")
        self.spin_offset_hour_val = tk.StringVar(value="0")
        self.spin_offset_hour = tk.Spinbox(
            self.root,
            from_=0,
            to=10,
            textvariable=self.spin_offset_hour_val,
            justify=tk.CENTER,
            state="readonly",
        )
        ...
        self.combo_fps = ttk.Combobox(
            self.root,
            state="readonly",
            values=self.combo_fps_values,
            textvariable=self.combo_fps_val,
        )
        ...
        self.btn_save_srt = tk.Button(
            self.root,
            text="Save SRT (optional: apply sync fix to SRT)",
            command=self._on_click_save_srt,
        )

        row_widget = 0
        self.btn_open_srt.grid(
            row=row_widget, column=0, columnspan=1, sticky="ew", pady=10
        )
        row_widget += 1
        self.label_offset_hour.grid(
            row=row_widget, column=0, columnspan=1, sticky="ew", pady=10
        )
        self.spin_offset_hour.grid(
            row=row_widget, column=1, columnspan=2, sticky="ew", pady=10, padx=5
        )
        row_widget += 1
        self.label_fps.grid(
            row=row_widget, column=0, columnspan=1, sticky="ew", pady=10
        ...
        row_widget += 1
        self.txt_youtube_chapters.grid(
            row=row_widget, column=0, columnspan=3, sticky="nsew"
        )
        self.root.rowconfigure(row_widget, weight=1)  # scrolledtext expand to fill
        row_widget += 1
        self.btn_save_srt.grid(
            row=row_widget, column=0, columnspan=3, sticky="ew", pady=10, padx=10
        )
        ...
        # list of all the widgets in the left frame (in order), for grid (partially) and for binding click focus
        self.fr_btn_widgets = []
        ...
        # open config file
        config_path = SOURCE_DIR / "srt_to_youtube_chapters_config.json"
        try:
    ...
    def apply_srt_offset_multiplier(self):
        ...
        offset_hour = int(self.spin_offset_hour_val.get())
        multiplier = (
            1 if self.combo_fps_val.get() == self.combo_fps_values[0] else 1001 / 1000
        )
        srt_utils.srt_drift_fix_NTSC(
            srt_lines, offset_hour=offset_hour, multiplier=multiplier
        )
        return srt_lines
    ...
    def _on_click_open_srt(self):
        ...
        if srt_path != "":
            ...
            self.save_config()
            self.root.title(
                "SRT to YouTube Chapters - " + os.path.basename(self.srt_path)
            )

            with open(self.srt_path, encoding="utf8") as f:
    ...
    def _on_click_save_srt(self):
        try:
            ...
            messagebox.showinfo(
                "Save to SRT",
                "The offset and drift fix have successfully been saved to the opened SRT.",
            )
