...
def _get_youtube_x(insta_x, fa_youtube_width, fa_insta_width):
    """ Get YouTube icon's x position given Instagram x
...
def _get_insta_xy_bottom_right(img_size, fa_youtube_size, fa_insta_size, youtube_name_size, insta_id_size, font_size):
    ...
    signature_width = text_x + max(youtube_name_size[0], insta_id_size[0]) - min(let_youtube_x, let_insta_x)
...
def watermark_signature(img, insta_id = "kiyoon0", youtube_name = "Kiyoon Kim"):
    ...
    insta_x, insta_y = _get_insta_xy_bottom_right(img.size, fa_youtube_size, fa_insta_size, youtube_name_size, insta_id_size, font_size)
